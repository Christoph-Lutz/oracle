...
/*
 * Purpose:
 *   Trace disk write requests slower than the given threshold.
 *
 * Author:
 *   Christoph Lutz
 *
...
@@BEGIN
{
    REQUEST_SIZE = $1;
    THRESHOLD_MS = $2;
    printf("Tracing slow write I/O requests (request_size: %d, threshold_ms: %d) ... hit ^C to stop\n", REQUEST_SIZE, THRESHOLD_MS);
}

@@io:disk:iostart:write:*
when(__iobuf->bcount == REQUEST_SIZE)
{
    ts_entry[__iobuf->bufid] = (long long) timestamp();
}

@@io:disk:iodone:write:*
